...
export default function MyPage() {
  ...
  async function fetchOrders(userId: string) {
    const { data } = await supabase.from('orders').select('*, products(name)').eq('user_id', userId).order('created_at', { ascending: false })
    if (data) setOrders(data)
  }

  async function fetchRequests(userId: string) {
    const { data } = await supabase.from('point_requests').select('*').eq('user_id', userId).order('created_at', { ascending: false })
  ...
  return (
    <main style={{background:'#5DD85A',minHeight:'100vh',maxWidth:'430px',margin:'0 auto',paddingBottom:'64px',fontFamily:'inherit'}}>
      ...
      <div style={{background:'#fff',margin:'0 16px 14px',padding:'14px 16px'}}>
        <div style={{fontSize:'14px',fontWeight:700,color:'#111',marginBottom:'10px'}}>미션 현황</div>
        {requests.length === 0 && <div style={{fontSize:'13px',fontWeight:700,color:'#888',textAlign:'center',padding:'8px'}}>아직 참여한 미션이 없어요</div>}
        {requests.map(r=>(
          <div key={r.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:'1px solid #eee'}}>
            <span style={{fontSize:'13px',fontWeight:700,color:'#111'}}>{missionLabel(r.mission_type)}</span>
      ...
      <div style={{background:'#fff',margin:'0 16px 14px',padding:'14px 16px'}}>
        <div style={{fontSize:'14px',fontWeight:700,color:'#111',marginBottom:'10px'}}>구매 내역</div>
        {orders.length === 0 && <div style={{fontSize:'13px',fontWeight:700,color:'#888',textAlign:'center',padding:'8px'}}>구매 내역이 없어요</div>}
        {orders.map(o=>(
          <div key={o.id} style={{display:'flex',justifyContent:'space-between',padding:'8px 0',borderBottom:'1px solid #eee',fontSize:'13px'}}>
            <span style={{fontWeight:700,color:'#111'}}>{o.products?.name} {o.quantity}개</span>
            <span style={{fontWeight:700,color:'#E24B4A'}}>-{o.total_points.toLocaleString()}P</span>
          </div>
        ))}
      </div>
